            ...
            <div className={`Card ${Hidden ?  "hidden" : "flex flex-col "}` }>
                ...
                <picture style={{backgroundImage: "url('" + Image + "')"}} className='Card-Image'></picture>
                {
                    Item.Video ? <div className="grid grid-cols-2 gap-0">
                                ...
                                <button onClick={ () => { SetCurrentVideo(Item); SetOpen(true); } } className='Card-Button border-l-2'>
    ...
    function ShowCards({ArrayOfProjects}) {
        ...
        return (
            ...
            <ul className="w-full flex flex-wrap gap-8 justify-center pb-10">
                <li><button className={`${Filter == "All" ? "font-semibold text-xl" : "font-thin text-lg"} text-white`}   onClick={() => { SetFilter("All") }}>{(Language == "Portuguese") ? "Todos" : "All"}</button></li>
                <li><button className={`${Filter == "Web" ? "font-semibold text-xl" : "font-thin text-lg"} text-white`}   onClick={() => { SetFilter("Web") }}>{(Language == "Portuguese") ? "Sites" : "Web"}</button></li>
                <li><button className={`${Filter == "Game" ? "font-semibold text-xl" : "font-thin text-lg"} text-white`}  onClick={() => { SetFilter("Game") }}>{(Language == "Portuguese") ? "Desenvolvimento de Jogos" : "Game Development"}</button></li>
                <li><button className={`${Filter == "Other" ? "font-semibold text-xl" : "font-thin text-lg"} text-white`} onClick={() => { SetFilter("Other") }}>{(Language == "Portuguese") ? "Outros" : "Other"}</button></li>
            </ul>
